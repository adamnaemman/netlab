        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
            <div className="w-full max-w-4xl h-[85vh] bg-[#1a1a1a] rounded-3xl border-2 border-[#f0883e] flex flex-col overflow-hidden shadow-[0_0_50px_rgba(240,136,62,0.2)]">
                ...
                <div className="flex-1 flex overflow-hidden">
                    {/* Sidebar Steps */}
                    <div className="w-72 bg-[#141414] border-r border-[#333] p-6 space-y-6 overflow-y-auto">
                        <div className="space-y-4">
                            ...
                            {cablingSteps.map(s => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-xl border-2 transition-all flex gap-3 ${cablingProgress.includes(s.id)
                                            ? 'bg-[#f0883e]/10 border-[#f0883e] scale-[0.98]'
                                            : 'bg-[#000] border-[#333] opacity-60'
                                        }`}
                                >
                                    <span className="text-xl">{s.icon}</span>
                                    <span className={`text-xs font-bold leading-tight ${cablingProgress.includes(s.id) ? 'text-white' : 'text-[#888]'}`}>
                                        {s.text}
                                    </span>
                                </button>
                            ))}
                        ...
                        {cablingProgress.length === cablingSteps.length && step === 'cabling' && (
                            <button
                                ...
                                className="w-full py-4 bg-[#f0883e] text-black font-black rounded-xl shadow-[0_4px_0_#9a5020] active:translate-y-1 active:shadow-none transition-all animate-bounce"
                            >
                                START PUTTY →
                            </button>
                        )}
                    ...
                    {/* Main Stage */}
                    <div className="flex-1 bg-black relative flex flex-col p-8 items-center justify-center">
                        ...
                        {step === 'cabling' && (
                            <div className="text-center space-y-6">
                                ...
                                <h2 className="text-3xl font-black text-white italic">Hardware Setup Required</h2>
                                <p className="text-[#888] max-w-sm mx-auto">Sila lengkapkan prosedur cabling kat sidebar sebelah kiri untuk teruskan ke terminal configuration.</p>
                            </div>
                        )}

                        {step === 'putty-setup' && (
                            <div className="w-96 bg-[#c0c0c0] p-1 border-2 border-[#fff] border-r-[#808080] border-b-[#808080] shadow-xl text-black font-sans">
                                {/* PuTTY Window Title */}
                                <div className="bg-[#000080] p-1 flex justify-between items-center mb-4">
                                    <span className="text-white text-xs font-bold px-1">PuTTY Configuration</span>
                                ...
                                <div className="p-4 space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-4">
                                                <div className="flex-1">
                                                    ...
                                                    <input type="text" value={puttyConfig.serialLine} className="w-full border-2 border-[#808080] border-t-[#000] border-l-[#000] bg-white px-1 h-6" />
                                                </div>
                                                <div className="w-24">
                                                    <p className="text-[10px]">Speed</p>
                                                    <input type="text" value={puttyConfig.speed} className="w-full border-2 border-[#808080] border-t-[#000] border-l-[#000] bg-white px-1 h-6" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-end gap-2">
                                        <button
                                            ...
                                            className="px-6 py-1 bg-[#c0c0c0] border-2 border-[#fff] border-r-[#000] border-b-[#000] hover:bg-[#d0d0d0] text-xs font-bold"
                                        >
                                            Open
                                        </button>
                                        <button className="px-6 py-1 bg-[#c0c0c0] border-2 border-[#fff] border-r-[#000] border-b-[#000] text-xs font-bold">Cancel</button>
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="w-full h-full flex flex-col bg-black border-4 border-[#333] rounded-lg overflow-hidden group">
                                <div className="bg-[#333] p-1 flex justify-between items-center px-4">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <span className="w-1.5 h-1.5 rounded-full bg-red-500 opacity-20"></span>
                                    </div>
                                </div>
                                <div className="flex-1 p-6 font-mono text-sm overflow-y-auto text-[#c9d1d9] space-y-1">
                                    {terminalLines.map((line, i) => (
                                        <div key={i} className={
                                            line.type === 'system' ? 'text-[#8b949e] italic mb-4' :
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <input
                                            ...
                                            className="flex-1 bg-transparent border-none outline-none caret-[#7ee787]"
                                        />
                                    </div>
                                    <div ref={bottomRef} />
                                </div>
